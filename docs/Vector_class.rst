Vector
======

.. autoclass:: crossproduct.crossproduct.Vector
   :show-inheritance:

.. rubric:: Code Example

.. code-block:: python

   >>> from crossproduct import Vector
   >>> v = Vector(1,2)
   >>> print(v)
   Vector(1.0, 2.0)

.. Rubric:: Properties and Methods

.. autosummary::
   :toctree: _autosummary

   ~crossproduct.crossproduct.Vector.__add__
   ~crossproduct.crossproduct.Vector.__eq__
   ~crossproduct.crossproduct.Vector.__mul__
   ~crossproduct.crossproduct.Vector.__sub__
   ~crossproduct.crossproduct.Vector.angle
   ~crossproduct.crossproduct.Vector.coordinates
   ~crossproduct.crossproduct.Vector.cross_product
   ~crossproduct.crossproduct.Vector.dot
   ~crossproduct.crossproduct.Vector.index_largest_absolute_coordinate
   ~crossproduct.crossproduct.Vector.is_codirectional
   ~crossproduct.crossproduct.Vector.is_collinear
   ~crossproduct.crossproduct.Vector.is_opposite
   ~crossproduct.crossproduct.Vector.is_perpendicular
   ~crossproduct.crossproduct.Vector.length
   ~crossproduct.crossproduct.Vector.nD
   ~crossproduct.crossproduct.Vector.normalise
   ~crossproduct.crossproduct.Vector.opposite
   ~crossproduct.crossproduct.Vector.perp_product
   ~crossproduct.crossproduct.Vector.perp_vector
   ~crossproduct.crossproduct.Vector.render
   ~crossproduct.crossproduct.Vector.triple_product
   ~crossproduct.crossproduct.Vector.x
   ~crossproduct.crossproduct.Vector.y
   ~crossproduct.crossproduct.Vector.z
